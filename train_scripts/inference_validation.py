...
lbls_classes = {0: 'простой',
        1: 'вынужденная',
        2: 'сварка'}

classes_lbls = {'простой': 0,
        'вынужденная': 1,
        'сварка': 2}
...
if __name__ == "__main__":
    ...
    for _, r in tqdm(df_nine_left[df_nine_left['time'] <= 7200].iterrows()):
        ...
        predicts_score.append(predicted.pred_score.cpu().numpy())
    ...
    predicts_score = np.asarray(predicts_score)

    predicts_score[:, 0] = moving_average(predicts_score[:, 0], window_size)
    predicts_score[:, 1] = moving_average(predicts_score[:, 1], window_size)
    predicts_score[:, 2] = moving_average(predicts_score[:, 2], window_size)
    ...
    print(classification_report(result_df['gt_class_indx'], result_df['class_indx'], 
